...
//+------------------------------------------------------------------+
//| Crea un'etichetta visiva sul grafico per un buffer               |
//+------------------------------------------------------------------+
bool CreateBufferLabel(int index, string indicatorName, double value, int corner = CORNER_RIGHT_UPPER, int shift = 0)
{
   string labelName = "BuffLabel_" + indicatorName + "_" + IntegerToString(index);
   string labelText = GetBufferLabel(index, indicatorName) + ": " + DoubleToString(value, 5);
   color labelColor = GetBufferColor(index, indicatorName);
...
//+------------------------------------------------------------------+
//| Rimuove un'etichetta visiva dal grafico                          |
//+------------------------------------------------------------------+
bool RemoveBufferLabel(int index, string indicatorName)
{
   string labelName = "BuffLabel_" + indicatorName + "_" + IntegerToString(index);
   
   if(ObjectFind(0, labelName) >= 0)
   {
      return ObjectDelete(0, labelName);
   }
...
//+------------------------------------------------------------------+
//| Aggiorna tutte le etichette visive per un indicatore             |
//+------------------------------------------------------------------+
bool UpdateBufferLabels(string indicatorName, int handle, int numBuffers, int corner = CORNER_RIGHT_UPPER, int shift = 0)
{
   for(int i = 0; i < numBuffers; i++)
   {
      double buffer[];
      if(CopyBuffer(handle, i, shift, 1, buffer) > 0)
      {
         CreateBufferLabel(i, indicatorName, buffer[0], corner, shift);
      }
   }
...
color GetBufferColor(int index, string indicatorName)
{
   ...
   // Colori specifici per indicatori comuni
   
   // MACD
   if(StringFind(lname, "macd") != -1)
   {
      if(index == 0) return clrBlue;    // MACD Line
      if(index == 1) return clrRed;     // Signal Line
      if(index == 2) return clrLime;    // Histogram
   }
   ...
   if(StringFind(lname, "stoch") != -1)
   {
      if(index == 0) return clrBlue;    // %K
      if(index == 1) return clrRed;     // %D
      if(index == 2) return clrLime;    // Segnale
   }
   
   // Ichimoku
   if(StringFind(lname, "ichimoku") != -1)
   {
      if(index == 0) return clrRed;        // Tenkan-sen
      if(index == 1) return clrBlue;       // Kijun-sen
      if(index == 2) return clrLightGreen; // Senkou Span A
      if(index == 3) return clrCrimson;    // Senkou Span B
      if(index == 4) return clrLime;       // Chikou Span
   }
   
   // ADX
   if(StringFind(lname, "adx") != -1)
   {
      if(index == 0) return clrBlue;    // ADX
      if(index == 1) return clrLime;    // +DI
      if(index == 2) return clrRed;     // -DI
   }
   
   // Alligator
   if(StringFind(lname, "alligator") != -1)
   {
      if(index == 0) return clrBlue;    // Jaw (13)
      if(index == 1) return clrRed;     // Teeth (8)
      if(index == 2) return clrLime;    // Lips (5)
   }
